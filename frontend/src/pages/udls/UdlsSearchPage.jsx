...
function UdlsSearchPage({ embedded }) {
  ...
  const fetchData = async (searchTerm = "") => {
    try {
      const response = await fetch(
        `${BASE_URL}/permits/search/?search=${searchTerm}`
      );
  ...
  const content = (
    <div style={container}>
      ...
      <p style={subtitle}>Search Drivers Permits records in the UDLS database</p>
      ...
      <div style={resultsContainer}>
        {loading ? (
          <p style={infoText}>Loading UDLS records...</p>
        ) : !query ? (
         ...
         records.map((item) => (
            <div key={item.id} style={card}>
                ...
                <h3>{item.holder_name}</h3>   {/* was item.name */}
                <span style={{
                    ...statusBadge,
                    background: item.is_flagged ? "#e24b4a" : theme.primary
                }}>
                    {item.is_flagged ? "⚑ Flagged" : item.status}
                </span>
                </div>
                ...
                <p><b>Report Count:</b> {item.report_count}</p>
            </div>
            ))
        )}
      </div>
