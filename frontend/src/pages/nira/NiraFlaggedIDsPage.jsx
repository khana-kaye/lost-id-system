...
function NiraFlaggedIDsPage({ embedded }) {
  ...
  const fetchRecords = async () => {
    try {
      ...
      console.error("NIRA background compilation error:", err);
      setAllRecords([]);
    } finally {
      setLoading(false);
    }
  ...
  const content = (
    <div style={pageWrapper}>
      ...
      <div style={header}>
        <div>
          <h1 style={title}>⚑ NIRA Conflict Flagged IDs</h1>
          <p style={subtitle}>
            System detected duplicates. National IDs listed below have been registered more than once in the database.
          </p>
        </div>

        <button style={backBtn} onClick={() => navigate("/nira")}>
          ← Dashboard
        </button>
      </div>
